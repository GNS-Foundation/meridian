# Virtual Bank Phase B — B3 definition (Meridian) — REPORT ONLY

**Status:** DEFINITION / scoping, report-only — no SIM code, no harness. **Date:** 2026-09-19.
**Roadmap position:** B1 (spine, GRADUATED) → B2 (fraud + Gate-1 τ; B2a competence-fraud catch
GRADUATED, Gate-1 τ + identity-cost pending) → **B3 = KYC / collections / credit-committee HITL**
→ B4 (auditor/regulator) → B5 (treasury/capital) → B6 (console + OSS extraction). Builds on the
B1 governed loop and B2 identity/fraud work; does not block on B2 Gate-1 τ landing but assumes it.
...
candidate sub-domains, in dependency order — B3 should take **credit-committee HITL first** (the
...
- **Collections disposition** — a HITL action on a defaulted/late receivable (restructure, write-off,
  escalate), tied back to the B1/B2 outcome that produced the default.

B3 is still "one bank, synthetic, PII-free by construction, deterministic episode" — the B1
...
B1/B2 post `POST /v1/governed/decisions` (one actor: the agent). A HITL disposition has **two**
...
tag, conformance-enforced. B3 is the **first real producer** of that record. Confirm before build:

- **Is `cgr.disposition.v1` implemented on the runtime yet, or spec-only?** 0009 accepted the
  *design*; B3 needs the record class + its HITL approval route live (grafomem `grafomem.hitl.*`
  approval endpoints exist as a read-only governance view — confirm the write/attest path). If it is
  spec-only, B3's first step is the runtime record class + attest route (a grafomem PR), not SIM code.
- **Approver identity (gap 3a, unresolved in 0009).** A defensible HITL approval needs a *verified
  ...
  dependency (KYC/IDV is not built; PoH is proof-of-trajectory). B3 must decide: model the officer as
  a **synthetic assured identity** (a seeded Ed25519 approver key labelled "verified" for the SIM,
  with the assurance gap explicitly stubbed and documented), NOT claim real IDV. This keeps B3
...
Meridian is **PII-free by construction** (B1: entities are synthetic, Faker, no real personal
data). KYC is *about* personal identity. B3 must model KYC/collections **without** real PII:
...
subject ref*, never identity attributes. This is the same primitive B2/0009 already point to
...
5. **B2 Gate-1 τ** — assumed available for the agent side; not a hard blocker for the credit-committee
   HITL demo (which is about the human disposition, not agent calibration).

## Moat boundary (unchanged from B1/B2)
...
live Grafomem runtime. B3 keeps this: the officer's signature is minted via the runtime's HITL
attest path, not in the SIM.

## B3 Definition of Done (draft — to confirm with the operator)
...
- **Join-verify (B1 discipline):** the disposition joins to its proposal and (for collections) to
  the B1/B2 outcome that triggered it — no silent no-op; `cgr_n_pending` moves as expected.
...
2. **`cgr.disposition.v1`** — is the runtime record class + HITL attest route in scope for B3 (likely
   a grafomem PR precedes the SIM), or already available?
3. **Approver assurance** — accept a synthetic "verified" officer identity for B3 (gap 3a stubbed),
   or does B3 require the real verified-named-person path (much larger, pulls in KYC/IDV)?
4. **Where B3 sits vs B2 Gate-1 τ** — sequence B3 after Gate-1 τ lands, or in parallel?
