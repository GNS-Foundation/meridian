# Virtual Bank Phase B — SIM-B1 build brief (Meridian)

Stand up **Meridian**, a synthetic commercial + trade-finance bank, as a living tenant on Grafomem —
the spine only. SIM-B1 reproduces our prior offline validation *live*: synthetic underwriting agents make
judgment decisions that run the real governed loop on a dedicated tenant, and reputation moves on the
real runtime. Fraud (SIM-B2), KYC/collections/committee (SIM-B3), auditor/regulator (SIM-B4), treasury (SIM-B5), and
console/OSS (SIM-B6) come later.

## Confirmed ingestion mapping (SIM-B1 Step-0, approved)
- **Decision** → `POST /v1/governed/decisions`; **Outcome** → `POST /v1/governed/outcomes` (the Ulissy
  dogfood pattern). This is the path CGR scores — **no HITL, no per-decision human attest**. SIM-B1 **and**
  SIM-B2 use this direct path.
- **Gate-1 (τ)** — the CGR calibration/identity threshold (Sybil/farm resistance) — is a scoring-engine
  property, fully automatic, **no HITL**, and enters at **SIM-B2** over this same direct path.
- **HITL policy-gate + Ed25519 attest** (propose→gate→HITL→attest→execute) is the human path for
  consequential actions = the **credit-committee escalation**, entering at **SIM-B3** on the consequential
  subset only.
...
- `agent_key` = the agent's GEIANT Ed25519 pubkey; CGR groups by it; **no mint handshake** — keys are
  supplied at decision time. SIM-B1 mints one Ed25519 keypair per agent locally.
- Tenant key scopes include **`cgr:read`** (+ `decisions:read` for the substrate) so SIM-B1 reads
  reputation back.
...
## SIM-B1 Definition of Done (incl. the three added requirements)
...
## SIM-B2 forward note (not a SIM-B1 blocker)
Phase A's Gate-1 win depended on τ bound to a **costly** GEIANT identity. SIM-B1 minting free local
keypairs is fine (no fraud yet), but SIM-B2's Sybil demo is credible only if the sim models the cost of a
*calibrated* identity vs a cheap one. Design that identity-cost model before SIM-B2 builds.
